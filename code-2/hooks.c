...
#ifdef DEBUG_INVALID_OP
        pr_info("%s: [INVALID OPCODE HOOK] [%d] The Loader Elf is running... Address of the Ret instruction to be simulated= %px\n",
        MOD_NAME,
        current->pid,
        (void *)regs->ip);
#endif
...
#ifdef DEBUG_INVALID_OP
            pr_info("%s: [INVALID OPCODE HOOK] [%d] The Kernel level Stack is empty and it is not possible to validate any return address to the user stack\n",
            MOD_NAME,
            current->pid);

            pr_info("%s: [INVALID OPCODE HOOK] [%d] The return address on the top of the user stack is%px\n",
            MOD_NAME,
            current->pid,
            (void *)ret_addr_user);
#endif
...
#ifdef DEBUG_INVALID_OP
            pr_info("%s: [INVALID OPCODE HOOK] [%d]The machine instruction prior to that episode by the Return Address is actually one CALL\n",
            MOD_NAME,
            current->pid);
#endif
...
void my_spurious_handler(struct pt_regs *regs){
    ...
    if(IS_ERR(absolute_path) || strlen(absolute_path) != strlen(absolute_path_elf_loader) || strcmp(absolute_path_elf_loader, absolute_path)) {
        ...
        pr_info("%s: [MY SPURIOUS HOOK] [%d] MATCHING Application name --> Kernel was requested to simulate a call with Int Oxff@%px\n",
        MOD_NAME,
        current->pid,
        (void *)((unsigned char *)regs->ip - 2));

        pr_info("%s: [MY SPURIOUS HOOK] [%d]Rip value saved from the firmware is equal to%px\n",
        MOD_NAME,
        current->pid,
        (void *)regs->ip);

        pr_info("%s: [MY SPURIOUS HOOK] [%d] The address from which the metadata for the call is present is equal to%px\n",
        MOD_NAME,
        current->pid,
...
#ifdef DEBUG_SPURIOUS
        pr_info("%s: [MY SPURIOUS HOOK] [%d]The return address that must be saved on the user stack and on the Kernel Stack is%px\n",
        MOD_NAME,
        current->pid,
        (void *)ret_addr);
#endif
...
#ifdef DEBUG_SPURIOUS
        pr_info("%s: [MY SPURIOUS HOOK] [%d] The absolute address of the function to which the kernel will pass the control is%px\n",
        MOD_NAME,
        current->pid,
        (void *)ip_addr);
#endif
...
#ifdef DEBUG_SPURIOUS
        pr_info("%s: [MY SPURIOUS HOOK] [%d] User execution will resume from the address %px\n",
        MOD_NAME,
        current->pid,
        (void *)regs->ip);

        pr_info("%s: [MY SPURIOUS HOOK] [%d] Before writing the return address, the user staninter has the value %px\n",
        MOD_NAME,
        current->pid,
        (void *)regs->sp);
#endif
...
#ifdef DEBUG_SPURIOUS
        pr_info("%s: [MY SPURIOUS HOOK] [%d] The new value of the user staninter is equal to %px\n",
        MOD_NAME,
        current->pid,
        (void *)regs->sp);

        pr_info("%s: [MY SPURIOUS HOOK] [%d] The Return Address written on the User Stack at the address %px it is equal to%px\n",
        MOD_NAME,
        current->pid,
        (void *)regs->sp,
        (void *)ret_addr);
        ...
        if(sm->base == sm->top) {
             ...
             */
/*
            printk("[DEBUG MIX ADDRESS] [CALL FIRST] somma corrente = %ld      numero corrente pendenti = %d       valore stack pointer corrente = %px\n",
...
/*
            printk("[DEBUG MIX ADDRESS] [CALL FIRST] somma aggiornata = %ld      numero aggiornato pendenti = %d       valore stack pointer aggiornato = %px\n",
                sm->stack_frame_size_sum,
                sm->num_stack_frame_pend,
...
int install_kprobes(void) {
    ...
    pr_info("%s: [MODULE INFO TEST] [%d] Do exit register pointer %lx\n",
    MOD_NAME,
    current->pid,
...
void remove_probes(void){
    ...
    pr_info("%s: [MODULE INFO TEST] [%d] Do exit UNregister pointer %lx\n",
    MOD_NAME,
    current->pid,
    (unsigned long) &kp_do_exit);
