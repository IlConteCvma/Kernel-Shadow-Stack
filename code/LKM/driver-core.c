...
int kss_module_init(void) {
    ...
    dprint_info("%s: [MODULE INIT] [%d] Workqueue was successfully created\n", MOD_NAME, current->pid);
#endif
    ...
    dprint_info("%s: [MODULE INIT] [%d] The new knot /proc/%s it was successfully created\n", MOD_NAME, current->pid, "kss_node");
    return 0;
...
void kss_module_exit(void) {
    ...
    pr_info("%s: [MODULE EXIT] [%d] The node /proc/%s it was successfully removed\n",
             MOD_NAME,
             current->pid,
             "kss_node");
...
static long my_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    switch(cmd) {
        ...
        case SECURITY_METADATA:
            ...
            dprint_info_v("%s: [IOCTL] [%d] Request for allocation and initialization of safety metadata\n", MOD_NAME, current->pid);
            ...
            if(end_of_stack[0] == (unsigned long)MAGIC_NUMBER && end_of_stack[2] == (unsigned long)MAGIC_NUMBER && (void *)sm != NULL) {
                ...
                preempt_enable();
                dprint_info_v("%s: [IOCTL] [%d] I metadati di sicurezza sono stati già allocati ed inizializzati\n", MOD_NAME, current->pid);
        ...
        case INSTRUM_MAP:
            ...
            ht_item *data;
    #endif
            dprint_info_v("%s: [IOCTL] [%d] Attempt to recover the Map of Instrumentation Map user space...\n",
                MOD_NAME,
                current->pid);
...
no_ret:
            ...
            hash_for_each_possible(ht_kss, data, ht_list_next, (unsigned long)current->mm) {
                ...
                if((unsigned long)data->mm_address == (unsigned long)current->mm) {
                    ...
                    dprint_info_v("%s: [IOCTL] [%d] Setting of the instrument map successful in the hash table\n",
                    MOD_NAME,
                    current->pid);
