...
void del_item_stack(stack_item *item, security_metadata *sm) {
    ...
    dprint_info_v("%s: [INVALID OPCODE HOOK][DELETE ITEM STACK][%d] The element in the Kernel Stack that you want to eliminate: (%px, %px)\n",
    MOD_NAME,
    current->pid,
...
#endif
{
    ...
    /*
     * Iter on all couples present in the Kernel Stack looking for that containing the information to validate
...
#endif
{
    ...
    count_ret = 0;
#endif
    /* Iter on all couples present in the Kernel Stack to validate the corresponding Return Address on the user stack */
...
curr_item = top_stack;
    ...
    if(sm->base != NULL) {
        ...
        while(curr_item != NULL) {
            ...
            user_stack_address_kernel = curr_item->addr_stack_user;
            dprint_info_v("%s: [CHECK ALL RETURN ADDRESS] [%d] Correct return address on the Kernel Stack = %px\tposition on the user stack saved at the kernel level = %px\n",
            MOD_NAME,
            current->pid,
            (void *)ret_addr_kernel,
            (void *)user_stack_address_kernel);
            ...
            if(first) {
                ...
                dprint_info_v("%s: [CHECK ALL RETURN ADDRESS] [%d] Position on the user stack of the first return address (RET 1) = %px\n",
                MOD_NAME,
                current->pid,
                ...
                dprint_info_v("%s: [CHECK ALL RETURN ADDRESS] [%d] Position on the user stack of the latest return address RET N = %px\n",
                        MOD_NAME,
                        current->pid,
                        (void *)user_stack_address_kernel_last);

                dprint_info_v("%s: [CHECK ALL RETURN ADDRESS] [%d] Range Stack User containing the return addresses[%px , %px]\n",
                        MOD_NAME,
                        current->pid,
                        (void *)user_stack_address_kernel,
                        (void *)user_stack_address_kernel_last);
                ...
                dprint_info_v("%s: [CHECK ALL RETURN ADDRESS] [%d] The base of the Kernel buffer is equal to %px\n",
                        MOD_NAME,
                        current->pid,                
                        (void *)buffer);
...
#endif //CHECK_BUFFER_SIZE
#endif
                dprint_info_v("%s: [CHECK ALL RETURN ADDRESS] [%d] On the ends of this memory area I have the addresses %px and %px\n",
                MOD_NAME,
                current->pid,
            ...
            dprint_info_v("%s: [CHECK ALL RETURN ADDRESS] [%d] user_stack_address_kernel = %px\tmapping_base_address = %px\tuser_stack_address_kernel - mapping_base_address = %px\n",
            MOD_NAME,
            current->pid,
            (void *)user_stack_address_kernel,
            (void *)mapping_base_address,
