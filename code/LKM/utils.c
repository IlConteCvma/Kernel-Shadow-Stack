...
int check_0x06(unsigned long ret_instr_addr, security_metadata *sm) {
    ...
    for(i=0; i<ret_num; i++) {
        ...
        if(map_0x06[i] == ret_instr_addr) {
            ...
            dprint_info_v("%s: [CHECK 0x06] The 0x06 byte by address %px It was added by the Loader Elf\n",
                MOD_NAME,
                (void *)ret_instr_addr);
    ...
    dprint_info_v("%s: [ERRORE CHECK 0x06]The 0x06 byte by address %px It was not added by the Loader Elf\n",
        MOD_NAME,
        (void *)ret_instr_addr);
...
int check_int_0xFF(unsigned long call_instr_addr, security_metadata *sm) {
    ...
    for(i=0; i<call_num; i++) {
        if(map_int_0xFF[i] == call_instr_addr) {
            ...
            dprint_info_v("%s: [CHECK INT 0xFF] Instruction Int 0xff at address %px It was added by the Loader Elf\n",
                MOD_NAME,
    ...
    dprint_info_v("%s: [ERRORE CHECK INT 0xFF] Instruction Int 0xff at address %px It was not added by the Loader Elf\n",
    MOD_NAME,
    (void *)call_instr_addr);
